...
def get_matrix(vectorizer, list_of_filepaths, text_analyzer):
    ...
    if not os.path.isdir(os.path.join(os.getcwd(), 'friends-data-preprocess')):
        os.mkdir(os.path.join(os.getcwd(), 'friends-data-preprocess'))
    with open('./friends-data-preprocess/matrix.csv', 'w') as f:
        for row in X.toarray():
            f.write(','.join(map(str, row)) + '\n')
    with open('./friends-data-preprocess/vocabulary.txt', 'w') as f:
        f.write(','.join(vocabulary))
    
    return X.toarray(), vocabulary


def get_answers(X, vocabulary):
    ...
    for i, st in enumerate(X.transpose()):
        if min(st) > 0:
            print(vocabulary[i])
    ...
    for person in persons:
        ...
        for name in person:
            if name in vocabulary:
                person_freq += sum(X.transpose()[vocabulary.index(name)])
...
def main():
    if not os.path.isdir(os.path.join(os.getcwd(), 'friends-data-preprocess')):
        ...
        X, vocabulary = get_matrix(vectorizer, list_of_filepaths, text_analyzer)
    else:
        with open(os.path.join(os.getcwd(), 'friends-data-preprocess', 'matrix.csv')) as f:
            X = np.array([list(map(int, row.split(','))) for row in f.readlines()])
        with open(os.path.join(os.getcwd(), 'friends-data-preprocess', 'vocabulary.txt')) as f:
            vocabulary = f.read().split(',')
        
    get_answers(X, vocabulary)
